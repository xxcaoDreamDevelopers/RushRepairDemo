...
-(id) initWithFrame:(CGRect) frame Titles:(NSArray *) titles taskNumbers:(NSArray *)taskNumbers{
    if (self = [super initWithFrame:CGRectMake(frame.origin.x, frame.origin.y, frame.size.width, 50)]) {
        
 
        
        self.taskNumberArray = [NSMutableArray arrayWithArray:taskNumbers];
        ...
        [_handler setFrame:CGRectMake(LEFT_OFFSET, 8, 39, 39)];
        ...
        UIImage *image1 = [UIImage imageNamed:@"btn"];
...
//                                              blue:0 alpha:.9f].CGColor;
    ...
    CGContextFillRect(context, CGRectMake(LEFT_OFFSET, rect.size.height-32.5, rect.size.width-RIGHT_OFFSET-LEFT_OFFSET, 6));
...
//    CGContextRestoreGState(context);
    ...
    for (i = 0; i < titlesArr.count; i++) {
        ...
        CGContextFillEllipseInRect(context, CGRectMake(centerPoint.x-10, rect.size.height-39.5f, 20, 20));
...
//        tempPoint.y = rect.size.height - 30.5f;
//        centerPoint = tempPoint;
        
        //        CGPoint numberPoint = CGPointMake(centerPoint.x + 2.5, rect.size.height - 30.5f);
        
        UILabel *numLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 20, 20)];
        numLabel.textColor = [Common colorFromHexRGB:@"815f24"];
        if (i == titlesArr.count - 1) {
            numLabel.textColor = [Common colorFromHexRGB:@"ef2101"];
        }
        numLabel.textAlignment = NSTextAlignmentCenter;
        numLabel.font = [UIFont systemFontOfSize:10.0];
        NSString *desc = [NSString stringWithFormat:@"%d",[self.taskNumberArray[i] intValue]];
        numLabel.text = desc;
        numLabel.center = CGPointMake(centerPoint.x , rect.size.height-29.5f);
        [self addSubview:numLabel];
        [self.numLabArray addObject:numLabel];
    }

    
    [self  bringSubviewToFront:_handler];
...
-(void) animateTitlesToIndex:(int) index{
    ...
    self.btnLabel.text = [NSString stringWithFormat:@"%d",[self.taskNumberArray[index] intValue]];
    self.btnLabel.textColor = self.TopTitlesColor;
...
//            [lbl setCenter:CGPointMake(lbl.center.x, self.frame.size.height-55)];
            [lbl setTextColor:[Common colorFromHexRGB:@"666666"]];
...
-(NSMutableArray *)numLabArray{
    if (_numLabArray == nil) {
        _numLabArray = [NSMutableArray arrayWithCapacity:10];
    }
    return _numLabArray;
}
...
-(UILabel *)btnLabel{
    if (_btnLabel == nil) {
        ...
        _btnLabel.font = [UIFont systemFontOfSize:17.0];
    }
    return _btnLabel;
}

//分类工单数量集合
-(NSMutableArray *)taskNumberArray{
    if (_taskNumberArray == nil) {
        _taskNumberArray = [NSMutableArray arrayWithCapacity:10];
    }
    return _taskNumberArray;
}
